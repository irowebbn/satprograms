...
data = open('uuf50-01.log','r').read()
xs = []
ys = []
lines = data.splitlines()
max_c = [0]
max_rd = 0
t1 = ax1.text(0, 190, "Max clauses satisfied: " + str(max_c),bbox=dict(facecolor='red', alpha=0.5))
t2 = ax1.text(0, 160, "Recursion depth: 0\nClauses satisfied: 0", bbox=dict(facecolor='red', alpha=0.5))
...
def animate(i, max_c):
    if ys[i] > max_c[0]:
        max_c[0] = ys[i]
    t1.set_text("Max clauses satisfied: " + str(max_c[0]) + "/218" )
...
#plt.show()
ani.save('dpll_unsat_example.mp4', writer=writer)